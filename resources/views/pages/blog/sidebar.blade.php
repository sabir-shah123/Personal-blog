            ...
            @foreach ($popularposts as $post)
                <li class="collection-item">
                    <a href="{{ route('blog.show', $post->slug) }}" class="indigo-text text-darken-4">
                        <span class="truncate tooltipped" data-position="bottom"
                            data-tooltip="{{ $post->title }}">{{ $post->title }}</span>
                    </a>
                </li>
            @endforeach
        </ul>
    ...
    <div class="card-content">
        ...
        <ul>
            @foreach ($categories as $category)
                <li class="category-bg-image"
                    style="background-image:url({{ Storage::url('category/slider/' . $category->image) }});">
